...
当前代码使用统一版本号 `20260717-manual-notice`，用于规避 App WebView / CDN 缓存：

- `index.html` 首屏脚本会检查 URL 中的 `v` 参数，若不是当前版本，会自动 `location.replace()` 到同一路径并补齐 `v=20260717-manual-notice`。
- `laya.css`、`game-api.js`、`laya-game.js` 均带同一版本号 query。
- 排行榜 iframe 使用 `LEADERBOARD_VERSION = APP_VERSION`，会打开 `leaderboard.html?v=20260717-manual-notice&embedded=1`。
...
| 活动页 URL | `https://show.jd.com/n/QwMWVE53XAodKr0x/?pageKey=QwMWVE53XAodKr0x&v=20260717-manual-notice` |
| 分享缩略图 | `https://m.360buyimg.com/babel/jfs/t16171/127/2505983508/7852/4cfd7bdf/5abc8954N23307760.png` |
| 当前 JS/CSS 版本号 | `20260717-manual-notice` |
